...
export default function CertificationsAdminPage() {
  ...
  useEffect(() => {
    fetch('/api/certifications')
      ...
      .catch(() => console.error('Failed to fetch certifications'))
  }, [])

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this certification?')) return
    await fetch(`/api/admin/certifications?id=${id}`, { method: 'DELETE' })
    setCertifications((prev) => prev.filter((p) => p.id !== id))
  }
    ...
    <div className="min-h-screen bg-[#0b1220] text-white px-8 py-10">
      ...
      <div className="bg-[#111a2d] rounded-xl p-6 shadow-lg border border-cyan-700/30">
        {certifications.length === 0 ? (
          <p className="text-gray-400 text-center py-10">No certifications yet.</p>
        ) : (
            ...
            <tbody>
              {certifications.map((item, i) => (
                <tr key={item.id} className="border-b border-cyan-700/20 hover:bg-cyan-900/10 transition">
                  ...
                  <td className="py-3 px-2 text-gray-300">{item.issuer || '—'}</td>
                  <td className="py-3 px-2 text-gray-400">{item.date || '—'}</td>
